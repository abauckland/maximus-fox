...
#create array to hold encoded product information
    ...
    #for each line of data
      ...
      #create array for each product line of identity pair ids
        ...
        (3..(instance_column - 1)).each do |n|
            ...
            if headers[n] === "manufacturer"
              product_company = Company.where(:name => line[n]).first              
      ...
      #create array for each product line of peform pair ids
        perform_pair_array = []
        ((instance_column + 1)..headers.length).each do |m|
            ...
            unit = Unit.where(:text => units[i]).first
            standard = Standard.where(:ref => standards[m]).first
            performtxt = Performtxt.where(:text => line[m]).first
            ...
            check_value_type = Valuetype.where(:unit_id => unit.id, :standard_id => standard.id).first_or_create
            check_perform_value = Performvalue.where(:valuetype_id => check_value_type.id).first           
            check_performtxtarray = Performtxtarray.where(:performtxt_id => performtxt, :performvalue_id => check_perform_value).first
            ...
            if check_performtxtarray
              performvalue_id = check_performtxtarray.performvalue_id
                ...
                new_perform_value = Performvalue.create(:valuetype_id => check_value_type.id)
                new_performtxtarray = Performtxtarray.create(:performtxt_id => performtxt, :performvalue_id => new_perform_value.id)
                performvalue_id = new_performtxtarray.id          
            end
            ###find or create perform pair
            
            perform = Perform.where(:performkey_id => perform_key.id, :performvalue_id => performvalue_id).first_or_create
...
#        existing_product = Product.joins(:descripts => :identity).where('identities.id' => product_array[2]).group('products.id')
