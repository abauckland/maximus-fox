...
layout "guides"
 ...
 def download_guides
   ...
   guides = Guide.where(:id => params[:guidance_ids]).collect{|i| i.id}
   
   guides.each do |guide|
    #have to change config files to get app to send file
    send_file("#{Rails.root}/public#{guide.pdf.url.sub!(/\?.+\Z/, '') }", :type => 'application/pdf', :filename => guide.photo_file_name)   
   
   #record download
   #capture IP address
   @guide_downloads.ip = request.remote_ip
   #guidance id
   #date
   end
 end
